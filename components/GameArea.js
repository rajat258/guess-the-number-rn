...
function GameArea({randomNumber, setViewIsVisible}) {
    ...
    function CheckAnswer(text){
        ...
        fixedNumberArray.find((fixedNumber) => {
            console.log('FixedArrayText : ',fixedNumberArray.text)
            if(fixedNumberArray.text === fixedNumber)
            {
                alert('Check the list.');
            }
        })
        if(chances == 1){
            alert('You Lost','Answer :',{randomNumber},
            [{text: 'OKK', onPress: CloseGame()}]
            )
        }
        else if(fixedNumbers < 1  || fixedNumbers > 99){
            console.log('FixedArrayTexta : ');
            alert('Number ranges between 1 - 99.');
            // setFixedNumberArray(fixedNumberArray.slice(-1));
        }
